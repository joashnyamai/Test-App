...
export const TestSuites = () => {
  ...
  const [editingSuiteId, setEditingSuiteId] = useState<string | undefined>();
  ...
  return (
    <div className="space-y-6">
      <TestSuiteList onEdit={handleEditSuite} onCreate={handleCreateNewSuite} />
...
const TestSuiteList: React.FC<TestSuiteListProps> = ({ onEdit, onCreate }) => {
  ...
  const filteredAndSortedSuites = useMemo(() => {
    ...
    return [...filtered].sort((a, b) => {
      ...
      // Handle number sorting for testCases
  ...
  const handleDeleteSelected = () => {
    ...
    if (window.confirm(`Are you sure you want to delete ${selectedSuites.size} test suite(s)?`)) {
                  ...
                  <TableCell className="text-right">
                    <div className="flex justify-end gap-2">
                      <Button
                        variant="ghost"
                        size="sm"
                        onClick={() => navigate(`/test-cases?suiteId=${suite.id}`)}
